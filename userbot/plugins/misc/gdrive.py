...
from userbot import userbot, Message, Config, get_collection, pool
from userbot.utils import progress, humanbytes, time_formatter
from userbot.utils.exceptions import ProcessCanceled
...
class _GDrive(_DBase):
    ...
    def _upload_file(self, file_path: str, parent_id: str) -> str:
        ...
        body = {"name": file_name, "mimeType": mime_type, "description": "Uploaded using Userbot"}
        if parent_id:
    ...
    async def upload(self) -> None:
        ...
        if replied and replied.media:
            ...
            dl_loc = await userbot.download_media(
                ...
                progress=progress,
                progress_args=(
                    "trying to download", userbot, self._message, c_time
                )
            )
...
@userbot.on_cmd("gsetup", about={'header': "Setup GDrive Creds"})
async def gsetup_(message: Message):
    """gsetup"""
    await Worker(message).setup()


@userbot.on_cmd("gconf", about={
...
@userbot.on_cmd("gclear", about={'header': "Clear GDrive Creds"})
async def gclear_(message: Message):
    """gclear"""
    await Worker(message).clear()


@userbot.on_cmd("gset", about={
...
@userbot.on_cmd("greset", about={'header': "Reset parent id"})
async def greset_(message: Message):
    """greset"""
    await Worker(message).reset_parent()


@userbot.on_cmd("gfind", about={
...
@userbot.on_cmd("gempty", about={'header': "Empty the Trash"})
async def gempty_(message: Message):
    """gempty"""
    await Worker(message).empty()


@userbot.on_cmd("gget", about={
