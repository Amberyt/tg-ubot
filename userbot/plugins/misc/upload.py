...
from userbot.utils import progress, take_screen_shot, humanbytes

LOGGER = userbot.getLogger(__name__)
CHANNEL = userbot.getCLogger(__name__)

LOGO_PATH = 'resources/tg-ubot.png'
THUMB_PATH = Config.DOWN_PATH + "thumb_image.jpg"


@userbot.on_cmd("rename", about={
...
async def doc_upload(chat_id, path, del_path: bool):
    message: Message = await userbot.send_message(
        chat_id, f"`Uploading {path.name} ...`")
    start_t = datetime.now()
    c_time = time.time()
    thumb = await get_thumb()
    await userbot.send_chat_action(chat_id, "upload_document")
    try:
        msg = await userbot.send_document(
            chat_id=chat_id,
            document=str(path),
            thumb=thumb,
            caption=path.name,
            ...
            progress=progress,
            progress_args=(
                "uploading", userbot, message, c_time, str(path.name)
            )
        )
...
async def vid_upload(chat_id, path, del_path: bool):
    ...
    start_t = datetime.now()
    c_time = time.time()
    await userbot.send_chat_action(chat_id, "upload_video")
    try:
...
async def audio_upload(chat_id, path, del_path: bool):
    ...
    await userbot.send_chat_action(chat_id, "upload_audio")
    try:
...
async def finalize(chat_id: int, message: Message, msg: Message, start_t):
    ...
    await userbot.send_chat_action(chat_id, "cancel")
    if message.process_is_canceled:
        await message.edit("`Process Canceled!`", del_in=5)
    else:
        end_t = datetime.now()
