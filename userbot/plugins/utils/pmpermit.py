...
@userbot.on_cmd("allow", about={
    'header': "allows someone to contact",
    'description': "Ones someone is allowed, "
                   "Userbot will not interfere or handle such private chats",
...
async def get_id(message: Message):
    ...
    if message.input_str:
        ...
        try:
            userid = (await userbot.get_users(user)).id
        except Exception as e:
            await message.err(str(e))
    return userid


@userbot.on_filters(Filters.private & ~Filters.me & ~Config.ALLOWED_CHATS
                   & ~Filters.outgoing & ~allowAllFilter & ~Filters.service & ~Filters.bot)
async def uninvitedPmHandler(message: Message):
    user_dict = await userbot.get_user_dict(message.from_user.id)
    ...
    if message.from_user.id in pmCounter:
        ...
        await message.reply(
            noPmMessage.format_map(SafeDict(**user_dict)) + '\n`- Protected by userbot`')
        await asyncio.sleep(1)
        await CHANNEL.log(f"#NEW_MESSAGE\n{user_dict['mention']} has messaged you")


@userbot.on_filters(Filters.private & ~Config.ALLOWED_CHATS & Filters.outgoing & ~allowAllFilter)
async def outgoing_auto_approve(message: Message):
    ...
    await ALLOWED_COLLECTION.update_one(
        {'_id': userID}, {"$set": {'status': 'allowed'}}, upsert=True)
    user_dict = await userbot.get_user_dict(userID)
    await CHANNEL.log(f"**#AUTO_APPROVED**\n{user_dict['mention']}")


@userbot.on_cmd("pmguard", about={
...
@userbot.on_cmd("setpmmsg", about={
    'header': "Sets the reply message",
    'description': "You can change the default message which userbot gives on un-invited PMs",
    'options': {
...
@userbot.on_cmd("vpmmsg", about={'header': "Displays the reply message for uninvited PMs"})
