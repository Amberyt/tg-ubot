...
async def eval_(message: Message):
    ...
    async def aexec(code):
        exec("async def __aexec(userbot, message):\n "
             + '\n '.join(line for line in code.split('\n')))
        return await locals()['__aexec'](userbot, message)
    try:
        await aexec(cmd)
    except Exception:
...
@userbot.on_cmd("exec", about={
    'header': "run shell commands",
    'usage': "{tr}exec [commands]",
    'examples': "{tr}exec echo \"Userbot\""})
...
@userbot.on_cmd("term", about={
    'header': "run terminal commands",
    'usage': "{tr}term [commands]",
    'examples': "{tr}term echo \"Userbot\""})
