...
async def mentionadmins(message: Message):
    ...
    try:
        async for x in userbot.iter_chat_members(chat_id=chat_id, filter="administrators"):
            status = x.status
            u_id = x.user.id
            username = x.user.username or None
            full_name = (await userbot.get_user_dict(u_id))['flname']
            if status == "creator":
