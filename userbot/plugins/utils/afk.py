...
from userbot import userbot, Message, Filters, Config, get_collection
...
@userbot.on_filters(IS_AFK_FILTER & ~Filters.me & ~Filters.bot & (
    Filters.mentioned | (Filters.private & ~Filters.service & Config.ALLOWED_CHATS)))
async def handle_afk_incomming(message: Message) -> None:
    user_id = message.from_user.id
    chat = message.chat
    user_dict = await userbot.get_user_dict(user_id)
...
@userbot.on_filters(IS_AFK_FILTER & Filters.outgoing, group=-1)
