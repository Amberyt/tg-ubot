...
@userbot.on_cmd("nowelcome", about={
...
@userbot.on_cmd("dowelcome", about={
...
@userbot.on_cmd("delwelcome", about={
...
async def raw_set(message: Message, name, collection, chats):
    ...
    if not string and not file_:
        ...
        if file_:
            ...
            c_time = time.time()
            tmp_path = await userbot.download_media(
                message=file_,
...
async def raw_view(message: Message, name, collection):
    ...
    if found:
        liststr += f"**{(await userbot.get_chat(message.chat.id)).title}**\n"
...
async def raw_ls(message: Message, name, collection):
    ...
    async for c_l in collection.find({}, {'media': 0}):
        liststr += f"**{(await userbot.get_chat(c_l['_id'])).title}**\n"
...
async def send_proper_type(message: Message,
    ...
    if file_type == 'audio':
        ...
        msg = await userbot.send_audio(chat_id=message.chat.id,
                                      audio=media,
                                      file_ref=file_ref,
                                      caption=caption,
                                      duration=duration,
        ...
        msg = await userbot.send_animation(chat_id=message.chat.id,
                                          animation=media,
                                          file_ref=file_ref,
                                          caption=caption,
                                          duration=duration,
        ...
        msg = await userbot.send_photo(chat_id=message.chat.id,
                                      photo=media,
                                      file_ref=file_ref,
                                      caption=caption,
                                      reply_to_message_id=message.message_id)
        ...
        msg = await userbot.send_sticker(chat_id=message.chat.id,
                                        sticker=media,
                                        file_ref=file_ref,
                                        reply_to_message_id=message.message_id)
        if caption:
        ...
        if os.path.exists(media):
            duration = extractMetadata(createParser(media)).get("duration").seconds
        msg = await userbot.send_voice(chat_id=message.chat.id,
                                      voice=media,
                                      file_ref=file_ref,
                                      caption=caption,
                                      duration=duration,
        ...
        if os.path.exists(media):
            ...
            if not thumb:
                thumb = take_screen_shot(media, duration)
        msg = await userbot.send_video_note(chat_id=message.chat.id,
                                           video_note=media,
                                           file_ref=file_ref,
                                           duration=duration,
                                           thumb=thumb,
        ...
        if os.path.exists(media):
            ...
            if not thumb:
                thumb = take_screen_shot(media, duration)
        msg = await userbot.send_video(chat_id=message.chat.id,
                                      video=media,
                                      file_ref=file_ref,
                                      caption=caption,
                                      duration=duration,
