...
async def who_is(message: Message):
    ...
    if user_id:
        try:
            from_user = await userbot.get_users(user_id)
            from_chat = await userbot.get_chat(user_id)
        except:
            await message.err(
                text="`no valid user_id or message specified, do .help whois for more info`")
            return
    elif message.reply_to_message:
        from_user = await userbot.get_users(message.reply_to_message.from_user.id)
        from_chat = await userbot.get_chat(message.reply_to_message.from_user.id)
    else:
        await message.err(
            text="`no valid user_id or message specified, do .help whois for more info`")
        return
    if from_user or from_chat is not None:
        ...
        if from_user.photo:
            local_user_photo = await userbot.download_media(message=from_user.photo.big_file_id)
