...
async def lock_perm(message: Message):
    ...
    get_perm = await userbot.get_chat(chat_id)
    ...
    if lock_type == "all":
        try:
            await userbot.set_chat_permissions(chat_id, ChatPermissions())
            await message.edit(
    ...
    try:
        await userbot.set_chat_permissions(chat_id,
...
async def unlock_perm(message: Message):
    ...
    get_uperm = await userbot.get_chat(chat_id)
...
async def view_perm(message: Message):
    ...
    v_perm = await userbot.get_chat(message.chat.id)
    ...
    if v_perm is not None:
        try:
            ...
            if v_perm.photo:
                local_chat_photo = await userbot.download_media(
