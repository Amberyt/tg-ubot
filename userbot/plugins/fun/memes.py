...
@userbot.on_cmd("Oof$", about={'header': "Ooooof"}, trigger='')
...
@userbot.on_cmd("Hmm$", about={'header': "Hmmmmm"}, trigger='')
...
@userbot.on_cmd("clock$", about={'header': "kensar clock animation"})
...
async def slap_(message: Message):
    ...
    if not u_id:
        ...
        return
    info_dict = await userbot.get_user_dict(u_id)
...
@userbot.on_cmd("(yes|no|maybe|decide)$", about={
...
async def decide_(message: Message):
    ...
    await message.delete()
    await userbot.send_photo(chat_id=chat_id,
                            photo=path,
...
async def dice_gen(message: Message):
    ...
    await userbot.send_dice(message.chat.id, random_emo)
    await message.delete()
