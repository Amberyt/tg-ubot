...
from userbot.plugins import get_all_plugins
...
class Userbot(Methods):
    ...
    async def load_plugin(self, name: str) -> None:
        """ Load plugin to Userbot """
        _LOG.debug(_LOG_STR, f"Importing {name}")
        self._imported.append(
            importlib.import_module(f"userbot.plugins.{name}"))
    ...
    async def restart(self, update_req: bool = False) -> None:
        """ Restart the Userbot """
        _LOG.info(_LOG_STR, "Restarting Userbot")
        await self.stop()
        try:
    ...
    def begin(self) -> None:
        ...
        _LOG.info(_LOG_STR, "Idling Userbot")
        run(Userbot.idle())
        _LOG.info(_LOG_STR, "Exiting Userbot")
